...
(function(window) {
  ...
  function SoundManager(smURL, smID) {
    ...
    var SMSound,
      ...
      _is_firefox = _ua.match(/firefox/i),
      ...
      _tryInitOnFocus =
        _isSafari &&
        (typeof _doc.hasFocus === 'undefined' || !_doc.hasFocus()),
      _okToDisable = !_tryInitOnFocus,
      _flashMIME = /(mp3|mp4|mpa)/i,
      _emptyURL = 'about:blank',
      _overHTTP = _doc.location
        ? _doc.location.protocol.match(/http/i)
        : null,
        ...
        if (_fV > 8) {
          if (_tO.isMovieStar === null) {
            _tO.isMovieStar =
              _tO.serverURL ||
              (_tO.type
                ? _tO.type.match(_netStreamMimeTypes)
                : false) ||
    ...
    this.play = function(sID, oOptions) {
      ...
      if (!_didInit || !_s.ok()) {
        _complain(
          _sm + '.play(): ' + _str(!_didInit ? 'notReady' : 'notOK'),
        );
        return result;
      }
      if (!_idCheck(sID)) {
    ...
    this.canPlayURL = function(sURL) {
      ...
      if (!result && _needsFlash) {
        result =
          sURL && _s.ok() ? !!sURL.match(_s.filePattern) : null;
      }
      return result;
    };
    ...
    this.getSoundById = function(sID, _suppressDebug) {
      if (!sID) {
        throw new Error(
          _sm + '.getSoundById(): sID is null/undefined',
        );
      }
    ...
    SMSound = function(oOptions) {
      ...
      this.play = function(oOptions, _updatePlayState) {
        ...
        if (_t.paused && _t.position && _t.position > 0) {
          ...
          _t.paused = false;
          _t.position =
            typeof _t._iO.position !== 'undefined' &&
            !isNaN(_t._iO.position)
              ? _t._iO.position
              : 0;
          if (!_t.isHTML5) {
      ...
      this.pause = function(_bCallFlash) {
        if (
          _t.paused ||
          (_t.playState === 0 && _t.readyState !== 1)
        ) {
          return _t;
        }
        _t.paused = true;
      ...
      this.unmute = function() {
        ...
        if (!_t.isHTML5) {
          _flash._setVolume(
            _t.sID,
            hasIO ? _t._iO.volume : _t.options.volume,
          );
        } else if (_t._a) {
          _t._a.muted = false;
        }
      ...
      _resetProperties = function(retainPosition) {
        ...
        _t.bytesTotal = null;
        _t.duration =
          _t._iO && _t._iO.duration ? _t._iO.duration : null;
        ...
        if (!_t.isHTML5 && _fV > 8) {
          ...
          if (_iO.useEQData) {
            if (
              ...
              if (
                typeof oEQData.rightEQ !== 'undefined' &&
                oEQData.rightEQ
              ) {
    ...
    _getDocument = function() {
      return (
        _doc.body ||
        _doc._docElement ||
        _doc.getElementsByTagName('div')[0]
      );
    };
    _id = function(sID) {
      ...
      function _cp(m) {
        ...
        if (m instanceof Array) {
          for (i = 0, j = m.length; i < j && !isOK; i++) {
            if (
              _s.html5[m[i]] ||
              a.canPlayType(m[i]).match(_s.html5Test)
            ) {
              isOK = true;
              _s.html5[m[i]] = true;
              _s.flash[m[i]] = !!(
    ...
    _disableObject = function(o) {
      ...
      for (oProp in o) {
        if (
          o.hasOwnProperty(oProp) &&
          typeof o[oProp] === 'function'
        ) {
          o[oProp] = _doNothing;
        }
      }
    ...
    _setVersionInfo = function() {
      ...
      var isDebug =
        _s.debugMode || _s.debugFlash ? '_debug.swf' : '.swf';
      if (
        _s.useHTML5Audio &&
        !_s.html5Only &&
      ...
      _s.filePattern =
        _s.filePatterns[_fV !== 8 ? 'flash9' : 'flash8'];
      _s.movieURL = (_fV === 8
        ? 'soundmanager2.swf'
    ...
    _processOnEvents = function(oOptions) {
      ...
      var status = {
          success:
            oOptions && oOptions.ignoreInit ? _s.ok() : !_disabled,
        },
        srcQueue =
          oOptions && oOptions.type
            ? _on_queue[oOptions.type] || []
            : [],
        queue = [],
        i,
        j,
          ...
          if (_s.audioFormats.hasOwnProperty(item)) {
            if (
              (_s.audioFormats[item].required &&
                !_s.html5.canPlayType(_s.audioFormats[item].type)) ||
              _s.flash[item] ||
    ...
    _createMovie = function(smID, smURL) {
      ...
      smURL = _s.url;
      _s.wmode =
        !_s.wmode && _s.useHighPerformance ? 'transparent' : _s.wmode;
      if (
        _s.wmode !== null &&
        (_ua.match(/msie 8/i) ||
          (!_isIE && !_s.useHighPerformance)) &&
    ...
    _initComplete = function(bNoDisable) {
      ...
      if (!wasTimeout) {
        ...
        if (_disabled) {
          error = {
            type:
              !_hasFlash && _needsFlash ? 'NO_FLASH' : 'INIT_TIMEOUT',
          };
        }
      }
      if (_disabled || bNoDisable) {
