...
const TorchParticles: React.FC<Props> = ({ color, density, radius }: Props) => {
  ...
  const setPosition = React.useCallback(() => {
    ...
    [...Array(density)].forEach((_, i) => {
      const x = (Math.random() * radius - radius / 2) / 4;

      const y = Math.random() * radius - radius / 2;

      const z = (Math.random() * radius - radius / 2) / 4;
